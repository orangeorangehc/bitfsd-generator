...
"""
    ...
    def test_install_uses_declared_dependencies_without_torch(self):
        import tomllib
        result = self.run_setup()
        self.assertNotEqual(result.returncode, 0)  # Fake installation leaves dependencies absent.
        requirements = tomllib.loads((self.project / "pyproject.toml").read_text())["project"]["dependencies"]
        self.assertEqual(self.calls()[-1][4:], requirements)
        self.assertFalse(any("torch" in arg for call in self.calls() for arg in call))
